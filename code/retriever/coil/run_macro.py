...
from arguments import (
    ModelArguments,
    DataArguments,
    COILTrainingArguments as TrainingArguments,
    WandBArguments,
)
from marco_datasets import (
    GroupedMarcoTrainDataset,
    MarcoPredDataset,
    MarcoEncodeDataset,
)
...
def main():
    ...
    trainer = Trainer(
        ...
        data_collator=QryDocCollator(
            tokenizer, max_q_len=data_args.q_max_len, max_d_len=data_args.p_max_len
        ),
    )
        ...
        if os.path.exists(data_args.rank_score_path):
            if os.path.isfile(data_args.rank_score_path):
                raise FileExistsError(
                    f"score file {data_args.rank_score_path} already exists"
                )
            else:
    ...
    if training_args.do_encode:
        if training_args.local_rank > -1:
            raise NotImplementedError(
                "Encoding with multi processes is not implemented."
            )
        ...
        encode_loader = DataLoader(
            ...
            collate_fn=DataCollatorWithPadding(
                tokenizer, max_length=data_args.p_max_len, padding="max_length"
            ),
            shuffle=False,
            drop_last=False,
        ...
        np.save(
            os.path.join(data_args.encoded_save_path, f"cls_pids"), np.array(all_pids)
        )
        np.save(os.path.join(data_args.encoded_save_path, f"cls_reps"), all_cls)
        offset_dict = {}
        tok_all_ids = []
        tok_all_reps = []
        _offset = 0
        ...
        with open(
            os.path.join(data_args.encoded_save_path, f"offsets.pkl"), "wb"
        ) as pf:
