...
def main():
    ...
    if args.as_torch:
        torch.save(
            torch.tensor(reps_by_query), os.path.join(args.save_to, "tok_reps.pt")
        )
    else:
    ...
    if args.as_torch:
        ...
        torch.save(
            torch.tensor(query_cls_reps), os.path.join(args.save_to, "cls_reps.pt")
        )
        torch.save(
            torch.tensor(query_cls_ids), os.path.join(args.save_to, "cls_ex_ids.pt")
        )
    else:
        copyfile(
            os.path.join(args.dir, "cls_reps.npy"),
            os.path.join(args.save_to, "cls_reps.npy"),
        )
        copyfile(
            os.path.join(args.dir, "cls_pids.npy"),
            os.path.join(args.save_to, "cls_ex_ids.npy"),
        )
