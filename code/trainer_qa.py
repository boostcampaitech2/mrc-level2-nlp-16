...
class QuestionAnsweringTrainer(Trainer):
    ...
    def evaluate(
        ...
        if self.post_process_function is not None and self.compute_metrics is not None:
            ...
            # Add eval_loss to metric
            start_logits = torch.Tensor(output.predictions[0])
            end_logits = torch.Tensor(output.predictions[1])
            start_positions = torch.LongTensor(eval_dataset["start_positions"])
            end_positions = torch.LongTensor(eval_dataset["end_positions"])
